...
func TestFreshSchemaStartsAtCurrentVersion(t *testing.T) {
	...
	if version != 85 {
		t.Fatalf("schema version = %d, want 85", version)
	}
}
...
func TestMigrateV63AddsProviderSyncProgress(t *testing.T) {
	...
	if version != 85 || current != 0 || startedAt.Valid {
		t.Fatalf("migrated sync progress = version:%d current:%d started:%v, want 85/0/NULL", version, current, startedAt.Valid)
	}
}
...
func TestMigrateV64AddsOutgoingRetrySchedule(t *testing.T) {
	...
	if version != 85 || nextAttempt.IsZero() {
		t.Fatalf("migrated outgoing retry = version:%d next:%v, want 85/non-zero", version, nextAttempt)
	}
}
...
func TestMigrateV75AddsGmailMessageFetchQueue(t *testing.T) {
	...
	if queued != 1 || version != 85 {
		t.Fatalf("migrated Gmail queue rows=%d version=%d, want 1/85", queued, version)
	}
}
