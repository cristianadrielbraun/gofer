...
const authChallengesV83Table = `CREATE TABLE %s (
	id TEXT PRIMARY KEY,
...
func migrateV82ToV83(tx *sql.Tx) error {
	exists, err := tableExistsTx(tx, "auth_challenges")
	if err != nil {
		return err
	}
	if !exists {
		if _, err := tx.Exec(fmt.Sprintf(authChallengesV83Table, "auth_challenges")); err != nil {
			return fmt.Errorf("create v83 authentication challenges: %w", err)
		}
		if err := createAuthChallengeV80Indexes(tx); err != nil {
			return err
		}
		return markSchemaVersion(tx, 83)
	}
	...
	if exists, err := tableExistsTx(tx, "auth_challenges_v82_old"); err != nil {
		return err
	} else if exists {
		return fmt.Errorf("temporary v82 authentication challenge table already exists")
	}
	if _, err := tx.Exec(`ALTER TABLE auth_challenges RENAME TO auth_challenges_v82_old`); err != nil {
		return fmt.Errorf("rename v82 authentication challenges: %w", err)
	}
	if _, err := tx.Exec(fmt.Sprintf(authChallengesV83Table, "auth_challenges")); err != nil {
		return fmt.Errorf("create v83 authentication challenges: %w", err)
	}
	if _, err := tx.Exec(`
		...
		SELECT id, user_id, session_id, challenge_hash, nonce_hash, purpose, origin,
		       attempts, max_attempts, payload_ciphertext, created_at, expires_at, consumed_at
		FROM auth_challenges_v82_old`); err != nil {
		return fmt.Errorf("copy v82 authentication challenges: %w", err)
	}
	if _, err := tx.Exec(`DROP TABLE auth_challenges_v82_old`); err != nil {
		return fmt.Errorf("drop v82 authentication challenges: %w", err)
	}
	if err := createAuthChallengeV80Indexes(tx); err != nil {
		return err
	}
