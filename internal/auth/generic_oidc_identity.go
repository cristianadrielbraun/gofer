	...
	result, err := m.completeFederatedPrimaryAuthenticationWithTransition(
		...
		func(tx *sql.Tx, now time.Time) error {
			...
			updated, err := tx.ExecContext(ctx, `
				...
				claims.DisplayEmail(), boolInt(claims.DisplayEmailVerified()), now, identityID, user.ID,
				oidcIdentityProvider, claims.Issuer, claims.Subject,
			)
			if err != nil {
				return fmt.Errorf("update OIDC identity use: %w", err)
			}
			changed, err := updated.RowsAffected()
			if err != nil {
				return fmt.Errorf("count OIDC identity update: %w", err)
			}
			if changed != 1 {
...
func (m *Manager) CompleteOIDCIdentityLink(
	ctx context.Context,
	challengeToken, sessionToken, code, userAgent string,
) (*FederatedIdentitySummary, error) {
	challenge, draft, claims, err := m.verifyOIDCCallback(
	...
	err = m.runSecurityTransition(ctx, SecurityTransitionIdentityChange, func(tx *sql.Tx) error {
		currentChallenge, currentDraft, nonceHash, err := m.currentOIDCAuthorizationChallenge(
			ctx, tx, challengeToken, ChallengePurposeFederatedLink,
		)
		if err != nil || currentChallenge.ID != challenge.ID || !sameOIDCLoginDraft(currentDraft, draft) ||
		...
		inserted, err := tx.ExecContext(ctx, `
			...
			identityID, currentSession.UserID, oidcIdentityProvider, claims.Issuer,
			claims.Subject, claims.DisplayEmail(), boolInt(claims.DisplayEmailVerified()), now, now,
		)
		if err != nil {
		...
		if err != nil {
			return fmt.Errorf("count OIDC identity insert: %w", err)
		}
		result := "linked"
		success := 1
		...
		if insertedCount == 0 {
			...
			if err := tx.QueryRowContext(ctx, `
				...
				return fmt.Errorf("load conflicting OIDC identity: %w", err)
			}
			if existingUserID != currentSession.UserID {
				...
				UPDATE auth_identities SET email = ?, email_verified = ?
				WHERE id = ? AND user_id = ?`,
				claims.DisplayEmail(), boolInt(claims.DisplayEmailVerified()), storedIdentityID, currentSession.UserID,
			); err != nil {
				return fmt.Errorf("refresh linked OIDC identity: %w", err)
			} else {
		...
		if err != nil {
			return fmt.Errorf("consume OIDC identity-link challenge: %w", err)
		}
		consumedCount, err := consumed.RowsAffected()
		if err != nil || consumedCount != 1 {
			return ErrPreAuthChallengeInvalid
		}
		metadata := `{"provider":"oidc","result":"` + result + `"}`
		if _, err := tx.ExecContext(ctx, `
			...
			return fmt.Errorf("record OIDC identity link: %w", err)
		}
		return nil
	})
	if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("load linked OIDC identity: %w", err)
	}
	return identity, nil
}

// UnlinkOIDCIdentity removes one exact application-login identity. It does not
// inspect or mutate any mailbox account or mailbox OAuth credential.
func (m *Manager) UnlinkOIDCIdentity(
	ctx context.Context,
	sessionToken string,
	identityID string,
	userAgent string,
	...
	if err != nil {
		return nil, fmt.Errorf("validate OIDC identity-removal relying party: %w", err)
	}
	...
	err = m.runSecurityTransition(ctx, SecurityTransitionIdentityChange, func(tx *sql.Tx) error {
		...
		if provider != oidcIdentityProvider {
			return ErrFederatedIdentityUnknown
		}
		...
		removed, err := tx.ExecContext(ctx, `
			...
			identityID, current.UserID, oidcIdentityProvider,
		)
		if err != nil {
...
func (m *Manager) validOIDCIdentity(claims *OIDCIDTokenClaims) bool {
	return boundedOIDCIdentityClaims(claims) && claims.Issuer == m.config.OIDCLoginIssuer
}
