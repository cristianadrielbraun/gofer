...
func (h *Handler) handleOIDCRedirect(w http.ResponseWriter, r *http.Request) {
	...
	start, err := h.auth.BeginOIDCLogin(r.Context())
	if err != nil {
		log.Printf("OIDC application login could not start: reason=challenge_initialization_failed")
		http.Error(w, "failed to initialize authentication", http.StatusInternalServerError)
		return
	}
...
func (h *Handler) handleOIDCCallback(w http.ResponseWriter, r *http.Request) {
	...
	if preAuthToken != "" {
		callbackPurpose, err := h.auth.GetOIDCCallbackPurpose(r.Context(), preAuthToken)
		if err == nil {
			purpose = callbackPurpose
		} else if !errors.Is(err, auth.ErrPreAuthChallengeInvalid) {
			log.Printf("OIDC authorization callback purpose lookup failed")
			h.rejectOIDCCallback(w, r, purpose, preAuthToken, "challenge_lookup_failed", false)
			return
		}
	}
	if !h.auth.IsCanonicalOIDCCallbackRequest(r) {
		h.rejectOIDCCallback(w, r, purpose, preAuthToken, "callback_origin_invalid", true)
		return
	}
	if preAuthToken == "" {
		h.rejectOIDCCallback(w, r, purpose, "", "challenge_missing", false)
		return
	}
	if !auth.PreAuthTokensMatch(r.URL.Query().Get("state"), preAuthToken) {
		h.rejectOIDCCallback(w, r, purpose, preAuthToken, "state_mismatch", true)
		return
	}
	...
	if code == "" {
		...
		h.rejectOIDCCallback(w, r, purpose, preAuthToken, reason, true)
		return
	}
	if purpose == auth.ChallengePurposeFederatedLink {
		_, err := h.auth.CompleteOIDCIdentityLink(
		...
		if err != nil {
			...
			log.Printf("OIDC identity link rejected: reason=%s", reason)
			http.Redirect(w, r, "/settings/security?oidc_link_failed=1", http.StatusSeeOther)
			return
		}
		http.Redirect(w, r, "/settings/security?oidc_linked=1", http.StatusSeeOther)
		return
	}
	if purpose != auth.ChallengePurposeFederatedLogin {
		h.rejectOIDCCallback(w, r, purpose, preAuthToken, "challenge_purpose_invalid", true)
		return
	}

	user, result, err := h.auth.HandleOIDCCallback(r.Context(), preAuthToken, code, r.UserAgent())
	auth.ClearPreAuthCookie(w, h.auth.Config().SecureCookies)
	if err != nil {
		log.Printf("OIDC application login rejected: reason=%s", auth.FederatedLoginReason(err))
...
func (h *Handler) rejectOIDCCallback(
	...
	if terminate && token != "" {
		if err := h.auth.TerminateOIDCAuthorizationChallenge(r.Context(), token); err != nil && !errors.Is(err, auth.ErrPreAuthChallengeInvalid) {
	...
	log.Printf("OIDC application login rejected: reason=%s", reason)
	http.Redirect(w, r, "/login?error=auth_failed", http.StatusSeeOther)
}
